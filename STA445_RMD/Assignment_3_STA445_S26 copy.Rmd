...
b.  We will test the function. Run prob1a.fun(5, 4). Did you get the correct result? Yes, I got 20.
...
d.  We will test the function. Run prob1c.fun(5, 4). Did you get the correct result? Run prob1c.fun(4, 5). Did you get the correct result? Yes, testing the function with values give me the correct results for both tests.
...
c.  Install the R package `microbenchmark`. We will use this to discover the average duration your function takes.
  The average duration the function takes is 22.67349 milliseconds.
...
d.  Instead of using a `for` loop, it might have been easier to use an `ifelse()` command. Rewrite your function to avoid the `for` loop and just use an `ifelse()` command. Verify that your function works correctly by producing a plot, and also run the `microbenchmark()`. Which version of your function was easier to write? Which ran faster?
  ifelse() function was easier to write and also much faster with an average of 2.23735 milliseconds.
...
b.  Test the function by running FizzBuzz(50). Did it work? Yes.
```{r}
FizzBuzz(50)
```
